...
| `AXIOMN_RATE_LIMIT_PER_MINUTE` | `60` | Max `POST /v1/intent` requests per client (per API key, or per IP without one) per fixed 60s window. Counts through the shared store (see `AXIOMN_REDIS_URL`), so the limit holds across instances. |
