...
`POST /intent` is the single point where an intent enters the system
(invariant 1: intent is the single entry point). The `/queue/...`
...
routing — `GET /queue/{id}` retrieves an escalated intent's eventual
answer, `POST /queue/{id}/answer` is where the human resolver delivers
it, `GET /queue` is the operator's worklist. Interactive API
documentation is auto-generated by FastAPI at `/docs` when the server is
running. The response schemas are the contract the SDK and all clients
depend on; changing them is a platform-layer event and, once the API is
versioned (see ROADMAP, Platform stage), a compatibility commitment.
